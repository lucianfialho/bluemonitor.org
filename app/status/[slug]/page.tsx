...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: `/api/og/status?name=${encodeURIComponent(service.name)}&domain=${encodeURIComponent(service.domain)}&status=${service.current_status || "up"}`,
          width: 1200,
          height: 630,
          alt: `Is ${service.name} Down?`,
        },
      ],
    },
    twitter: {
      ...
      title: `Is ${service.name} Down? — BlueMonitor`,
      description: `Check if ${service.name} is down right now. Real-time status monitoring for ${service.name}.`,
      images: [`/api/og/status?name=${encodeURIComponent(service.name)}&domain=${encodeURIComponent(service.domain)}&status=${service.current_status || "up"}`],
    },
  };
}
